...
> **A SaaS ships an admin dashboard.** Where this project is one, this document is the acceptance contract for
> an auth-gated operator surface, plus the structured-logging standard that runs project-wide. It is *not*
> genesis's own build-control dashboard, which is a window onto the build — this is a product feature of the
> app being built, the place an operator inspects and steers the live system. It is also what makes the running
> system observable at all, which is what the checks in [`TESTING.md`](./TESTING.md) read.
>
> Same discipline as everything else: auth-gated and least-privilege; every shown fact has a source — version,
> uptime and queue depth are read from the running system, never hardcoded; a panel whose data source is down
> degrades to a plainer correct view rather than blanking; and every panel is held to the project's reference.
> A confusing or lying admin panel is a defect.

---

## 0. Scope & the one rule

If `archetype = SaaS`, the app exposes an admin area meeting every section below before it is "done." If the
project is *not* a SaaS (CLI, library, batch tool), this whole document is `n/a` except §10 (Logging
standards), which applies to every project regardless of archetype.
...
reads and steers; it is not a second public API and it is not a backdoor.

## 1. Auth gate & least privilege
...
- **No secret ever rendered.** The admin UI shows that a key is *set*, its name, masked tail, last-rotated
  date — never the value. It does not read `.env`.
...
- List, search and filter each core entity (users, organizations/tenants, subscriptions, the project's
...
- Edit / create / soft-delete through the *same validated domain layer* the app uses — admin writes
  **parse, don't validate** at the boundary and respect every invariant; no raw SQL path that bypasses
  domain rules. An illegal edit is rejected with a legible reason, not silently coerced.
- Every mutation writes an audit-log entry (§7): who, what entity, before→after (PII-redacted per §10).
- Impersonation (if offered) is explicit, time-boxed, banner-visible to the operator, and audited as its
  own action — never silent.

## 3. System Status panel *(every fact has a source)*
...
with a muted style — it degrades, it does not fabricate.
...
degradation to manual refresh if the live channel drops — stale is labeled stale, never shown as
...
- Config edited here is the same config the app reads at runtime — no shadow store that drifts. Secrets
  are referenced by name only, never editable-as-plaintext in the UI.
- A flag has a safe default so a missing/unreadable flag degrades to a known-good behavior.
...
- **Self-lockout guard:** an admin cannot remove the last admin or strip their own critical capability
  without an explicit confirm — an illegal-state guard, not a runtime hope.
- Every role/permission change is audited with actor, target, before→after.

## 7. Audit log of admin actions *(distinct from §10 application logs)*
...
- Searchable and filterable by actor, action, target, date range; exportable with the same options as
  §10 (CSV / JSON / NDJSON, date-range).
- Redacted to the §10 standard — an audit entry records *that* a field changed and a safe summary, never a
  secret/PII value in the clear.
- Correlated to application logs via the shared `request_id` (§10) so an audited action can be traced into
  the operational logs end to end.
...
- The Status panel renders each dependency green/amber/red with last-checked time and latency; a
  ...
  green if the probe itself fails.
...
## 10. Logging standards — high-quality structured logging *(applies project-wide)*

Logs are evidence a test or operator reads, not decoration. This standard holds in every project genesis
builds, SaaS or not, and the admin dashboard's log viewer/exporter is just a window onto it.
...
- **Correlation / request IDs.** A request id is generated (or accepted from a trusted upstream header) at the
  ...
  to end — and ties application logs to the §7 audit log.
- **PII and secret redaction happens in one place, not at each call site.** A deny-by-default layer scrubs
  ...
  secret pattern survives serialization") and a redaction regression gets a failing test first.
- **Determinism & cost.** No secret in a URL/query that gets logged; sample or rate-limit high-volume events;
  keep logs cheap enough to leave on in prod. Inject the clock for `ts` so log assertions are testable.
...
- A date-range filter is required on every export — plus filter by level, `event`, `request_id`, actor.
...
The admin dashboard is done only when, observed:
...
- [ ] the System Status panel reads version, build hash, server time, uptime, env, dependency health
  live from the running system — no fabricated facts;
- [ ] live refresh works and degrades to manual on channel loss, with stale labeled stale;
- [ ] feature flags / config toggles flip from the UI, take effect without redeploy, are audited, and have
  safe defaults (§5);
- [ ] user/role management works with a last-admin / self-lockout guard;
...
- [ ] structured JSON logging with standard fields, honest levels, correlation IDs, and a tested
  ...
  itself redacted + audited;
- [ ] the admin area has its README and the panels meet the project's reference;
- [ ] the full Tier-B gate is green via `test-gate`.
